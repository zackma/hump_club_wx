...
function save(){
    ...
    isPassVal = $("input[name='isPass']:checked").val();
    var i = 0;
    if(eventDesVal != ""){
        loadingX("提交数据中...");
...
function postData(){
    ...
    var data = {
        ...
        eventId:$("#eventId").val(),
        declare:eventDesVal,               //事件描述
        caseType:caseTypeVal,    //事件类型
        isPass:isPassVal //0-虚报；1-实报
    };
    $.post(path+"/service/verifySubmit", data, function (data) {
        closeX();
        if (data.success) {
            ...
            alertX(data.msg);
            eventImages.serverId = [];
            localImages.serverId = [];
        }
    });
}